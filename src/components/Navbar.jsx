...
export default function Navbar({ isDarkMode, onToggleDarkMode }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 dark:bg-slate-900/90 dark:border-slate-800/80 transition-colors duration-300">
      <nav className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between gap-4">
          ...
          {NAV_LINKS.map((link) => (
            <li key={link}>
              <a 
                ...
                className="hover:text-primary-500 dark:hover:text-primary-400 transition-colors relative py-1 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 hover:after:w-full after:bg-primary-500 dark:after:bg-primary-400 after:transition-all after:duration-250"
              >
                {link}
              </a>
            </li>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            ...
            className="h-10 w-10 rounded-xl flex items-center justify-center text-ink-soft dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-100 dark:border-slate-800 transition-all"
          >
          ...
          <a
            href="#courses"
            aria-label="Search"
            className="h-10 w-10 rounded-xl flex items-center justify-center text-ink-soft dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-100 dark:border-slate-800 transition-colors"
          >
            <HiOutlineSearch className="h-5 w-5" />
          </a>

          {/* User Profile Avatar with custom ring */}
          <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-violet-500 to-primary-500 text-white flex items-center justify-center font-display text-sm font-bold shadow-md shadow-violet-500/20 cursor-pointer hover:scale-105 transition-transform">
            LK
          </div>
        </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <button
            onClick={onToggleDarkMode}
            aria-label="Toggle Dark Mode"
            className="h-9 w-9 rounded-lg flex items-center justify-center text-ink-soft dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
          >
            {isDarkMode ? <HiOutlineSun className="h-5 w-5 text-amber-400" /> : <HiOutlineMoon className="h-5 w-5" />}
          </button>
          ...
          <button
            className="h-9 w-9 flex items-center justify-center text-ink dark:text-white rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
